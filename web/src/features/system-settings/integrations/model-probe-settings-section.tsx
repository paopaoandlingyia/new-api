        ...
        <SettingsForm onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <div>
            <p className='text-muted-foreground text-xs'>
              {t(
                'Periodically sends one real request to each selected model so the model square can show a live status light. Probe traffic is generated by this site, so it reveals nothing about user activity.'
              )}
            </p>
            <p className='text-muted-foreground mt-1 text-xs'>
              {t(
                'Probes are billed as real upstream requests. Select models to monitor on the Model availability probe page.'
              )}
            </p>
          </div>
